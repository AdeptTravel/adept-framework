...
func New(global *sqlx.DB, idleTTL time.Duration, maxEntries int, lg *zap.SugaredLogger) *Cache {
	c := &Cache{
		globalDB:   global,
		idleTTL:    idleTTL,
...
func (c *Cache) Get(host string) (*Tenant, error) {
	...
	if v, ok := c.m.Load(host); ok {
		...
		c.log.Debugw("tenant cache hit", "tenant", host)
	...
	v, err, _ := c.sfg.Do(host, func() (interface{}, error) {
		...
		if v, ok := c.m.Load(host); ok {
			ent := v.(*entry)
			atomic.StoreInt64(&ent.lastSeen, time.Now().UnixNano())
			c.log.Debugw("tenant cache hit (after barrier)", "tenant", host)
			return ent.tenant, nil
		}

		start := time.Now()
		c.log.Infow("tenant loading", "tenant", host)

		ten, err := loadSite(context.Background(), c.globalDB, host)
		switch {
		case err == ErrNotFound:
			c.log.Warnw("tenant not found", "tenant", host)
			metrics.TenantLoadErrorsTotal.Inc()
			return nil, err
		case err != nil:
			c.log.Errorw("tenant load error", "tenant", host, "err", err)
			metrics.TenantLoadErrorsTotal.Inc()
			return nil, err
		}
